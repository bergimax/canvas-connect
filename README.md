...
- **Where**: a single EC2 instance (`deploy/cloudformation.yml`) running the same `docker-compose.yml` + `docker-compose.prod.yml` stack as local dev, plus Caddy as a reverse proxy for automatic HTTPS once a domain is set. Access is via SSM Session Manager only — no SSH port is open.
- **What ships**: `.github/workflows/ci-cd.yml`'s `build-and-push` job builds the app image once, after every test suite passes, and pushes it to an ECR repository (also provisioned by the CloudFormation stack) tagged with the commit SHA.
- **How it gets there**: the `deploy` job talks to the instance over SSM (`deploy/deploy.sh`) and asks it to pull that exact tag and restart the stack — no rebuilding on the server, so what runs in production is bit-for-bit what CI already tested.
- **How it's verified**: the same script polls the app's `/health` endpoint (which checks the database connection, not just that a process is listening) after every deploy, and **automatically rolls back** to the last known-good image (tracked in SSM Parameter Store) if the health check fails.

To actually turn this on: deploy `deploy/cloudformation.yml` to an AWS account, then set the `AWS_ROLE_ARN` secret and `AWS_REGION` / `DEPLOY_HEALTH_URL` variables on the GitHub repo (an OIDC role, so no long-lived AWS keys ever touch GitHub — the exact trust policy and IAM permissions needed are documented as comments directly above the `deploy` job). Every subsequent push to `main` deploys itself; until then, the pipeline detects the missing configuration and skips those jobs cleanly instead of failing.
...
deploy/
  cloudformation.yml  # EC2 instance + ECR repo + IAM roles
  deploy.sh            # what the deploy CI job runs over SSM
...
3. **`build-and-push`** (push to `main` only, after step 2 passes): builds the app image once and pushes it to ECR, tagged with the commit SHA — see [Deployment](#deployment).
4. **`deploy`**: redeploys the EC2 instance to that exact tag over SSM, health-checks it, and rolls back automatically on failure.

Steps 3 and 4 are written to no-op cleanly — not fail — until AWS is actually provisioned and its secrets/variables are set on the repo; see [Deployment](#deployment) for the exact list. `main` requires `backend-tests`, `frontend-tests`, and `integration-and-e2e` to pass before a pull request can merge.
...
1. Get AWS provisioned and the pipeline's `deploy` job actually running — everything is written and tested, it just needs an account and its secrets set (see [Deployment](#deployment)).
